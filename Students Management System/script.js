...
let student_data = [{"id":1,"first_name":"Chadwick","last_name":"Ayre","email":"cayre0@cam.ac.uk","gender":"Male","img_src":"https://robohash.org/corporisquiaperiam.png?size=50x50&set=set1","class":11,"marks":18,"passing":false,"city":"Moorreesburg"},
...
// Here I Have written Logic to render the table according my HTML and CSS code also the provided array.
...
function AtoZ(){
    ...
    student_data.sort(sortedArrayAtoZ);

    function sortedArrayAtoZ(a,b){
...
function ZtoA(){
    ...
    student_data.sort(sortedArrayZtoA);

    function sortedArrayZtoA(a,b){
        let NameA = a.first_name;
        let NameB = b.first_name;
...
function marksSort(){
    ...
    student_data.sort(shortArrayMarks);

    function shortArrayMarks(a,b){
        let MarkA = a.marks;
        let MarkB = b.marks;
...
function classSort(){
    ...
    student_data.sort(ShortClassAcending);

    function ShortClassAcending(a,b){
        let classA = a.class;
        let classB = b.class;
...
function passingSort(){
    ...
    student_data = student_data.filter((passed) => {
...
let table_head = document.getElementById("table_thead");


//This is the logic to render two different table one for MALE gender and another one for FEMALE.

function genderSort(){
    let maleGender = student_data.filter((GenderMale) => {
        return GenderMale.gender === 'Male';
    });

    let femaleGender = student_data.filter((GenderFemale) => {
        return GenderFemale.gender === 'Female';
    });

    table_body.innerHTML = ``

    maleGender.forEach((item) => {
    let passingStuden = (item.passing === false ? "Failed" : "Passed") 
    table_body.innerHTML += `
            <tr>
                <td>${item.id}</td>
...
function search(){
    ...
    //.replace(/\s+/g, '') this is the function i found from the net to remove all spaces before , after and between the words.
    //.trim() fuction can't be use here because i want to remove all the space in input value to compare it to stdanme.
    let trimInputValue = inputValue.replace(/\s+/g, '');

    console.log(trimInputValue);

    student_data = student_data.filter((item) => {
        let stdname = (item.first_name + item.last_name).toLowerCase();
        let trimName = stdname.replace(/\s+/g, '');

        let stdemail = item.email.toLowerCase();

        return trimName === trimInputValue || stdemail === trimInputValue;
    })
